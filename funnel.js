...
const storageKey='club_portal_funnel_v6';
function getData(){return getJSON(storageKey,[])} function setData(r){setJSON(storageKey,r)}

const SUPPORTERS = ["Justin","Jorick","Rainer","Birgitta","Aimee","Sybren","Demi","Tjardo","Marvin"];
const SUPPORTER_COLORS = {
  'Justin':'#4C8BF5','Jorick':'#34C759','Rainer':'#FF8A00','Birgitta':'#AF52DE',
  'Aimee':'#FF3B30','Sybren':'#50C8E8','Demi':'#FFCC00','Tjardo':'#8E8E93','Marvin':'#30D158'
};

let CLUBS = [];
async function loadClubs(){
  try{ const res=await fetch('data/clubs.json',{cache:'no-store'}); CLUBS = res.ok?await res.json():[]; } catch { CLUBS=[]; }
  const dl=document.getElementById('clubsList'); dl.innerHTML='';
...
function computeMetrics(rows){
  ...
  for(const r of rows){
    const d=derive(r);
    budgetSum+=d.budget; ownSum+=d.ownAmt; subSum+=d.subAmt; residSum+=d.residualAmt;
...
function renderMetrics(rows){
  ...
  tiles.innerHTML='';
  const el=(title,value,sub=null,icon='📊')=>{ const a=document.createElement('div'); a.className='tile'; a.style.alignItems='flex-start'; a.innerHTML=`<div class="icon">${icon}</div><div><h3>${escapeHtml(title)}</h3><p><strong>${escapeHtml(String(value))}</strong>${sub?`<br><span class="small">${escapeHtml(sub)}</span>`:''}</p></div>`; return a; };
...
function renderBoard(){
  ...
  for(const r of rows){
    ...
    const c=document.createElement('div'); c.className='card-item'; c.draggable=true; c.dataset.id=r.id;
    ...
    c.addEventListener('dragstart',e=>e.dataTransfer.setData('text/plain',r.id));
    c.addEventListener('click',()=>openModal(r.id));
    const zone=document.querySelector(`.dropzone[data-stage="${r.stage}"]`); if(zone) zone.appendChild(c);
...
function setupDnD(){
  document.querySelectorAll('.dropzone').forEach(z=>{
    ...
    z.addEventListener('drop',e=>{ e.preventDefault(); const id=e.dataTransfer.getData('text/plain'); const stage=z.dataset.stage; const rows=getData(); const i=rows.findIndex(x=>x.id===id); if(i<0) return; rows[i].stage=stage; setData(rows); renderBoard(); recalc(); });
  });
}
...
function bindClubPicker(form){
  ...
  input.addEventListener('change',()=>{
    ...
    if(hit){ hidden.value=hit.id; input.value=hit.naam; } else hidden.value='';
  });
}
...
function openModal(id=null){
  ...
  const rows=getData();
  let data={ id:uid(), clubId:'', vereniging:'', typeTraject:'', clubondersteuner:'', trajectbegeleider:'', startDate:'', endDateExpected:'', costBudget:'', financingType:'', financingPct:'', financingAmount:'', ownPct:'', ownAmount:'', stage:'intake', notes:'', lastUpdate:new Date().toISOString().slice(0,10) };
  ...
  const costBudget=form.querySelector('#costBudget'), financingPct=form.querySelector('#financingPct'), financingAmount=form.querySelector('#financingAmount'), ownPct=form.querySelector('#ownPct'), ownAmount=form.querySelector('#ownAmount'), coverageInfo=document.getElementById('coverageInfo');
...
document.getElementById('exportFunnelBtn').addEventListener('click',()=>{
  ...
  const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`funnel-v6-${new Date().toISOString().slice(0,10)}.json`;
